...
class RenkoBreakoutStrategy(BaseStrategy):
    """Renko-style breakout: consecutive bricks in same direction."""

    _STRATEGY_NAME = "renko_breakout"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        last_level = closes[0]
        for c in closes[1:]:
            if c >= last_level + brick:
                steps = int((c - last_level) / brick)
                bricks.extend([1] * steps)
                last_level += steps * brick
            elif c <= last_level - brick:
                steps = int((last_level - c) / brick)
                bricks.extend([-1] * steps)
                last_level -= steps * brick
        if len(bricks) < 3:
            return None
        last3 = bricks[-3:]
        if all(b == 1 for b in last3):
            direction = "long"
            streak = sum(1 for b in reversed(bricks) if b == 1)
        elif all(b == -1 for b in last3):
            direction = "short"
            streak = sum(1 for b in reversed(bricks) if b == -1)
        else:
            return None
        confidence = round(min(0.9, 0.5 + streak * 0.05), 3)
        entry = float(close.iloc[-1])
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"Renko streak={streak} bricks, direction={direction}",
        }
