...
class KylesLambdaStrategy(BaseStrategy):
    """Kyle lambda price impact measure for momentum trading."""

    _STRATEGY_NAME = "kyles_lambda"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        close = df["close"]
        volume = df["volume"]
        price_chg = close.diff().abs()
        lam = price_chg / (volume + 1e-9)
        lam_pct = lam.rolling(10).mean().rank(pct=True)
        lp = float(lam_pct.iloc[-1])
        if np.isnan(lp):
            return None
        if lp > 0.2:
            return None
        ret = float(close.pct_change(3).iloc[-1])
        if np.isnan(ret):
            return None
        direction = "long" if ret > 0 else "short"
        confidence = round(min(0.9, 0.5 + (0.2 - lp) * 1.5), 3)
        atr = self._calculate_atr(df)
        entry = float(close.iloc[-1])
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"Kyle lambda pct={lp:.2f}, low impact → momentum",
        }
