...
class GammaScalpingStrategy(BaseStrategy):
    """Gamma scalping proxy: buy dips and sell rips within ATR range."""

    _STRATEGY_NAME = "gamma_scalping"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        recent_low = float(close.rolling(10).min().iloc[-1])
        cur = float(close.iloc[-1])
        if np.isnan(recent_high) or np.isnan(recent_low) or atr == 0:
            return None
        pullback_from_high = recent_high - cur
        pullback_from_low = cur - recent_low
        if pullback_from_high > 0.5 * atr and pullback_from_high < 2.0 * atr:
            direction = "long"
            confidence = round(min(0.9, 0.5 + pullback_from_high / atr * 0.2), 3)
        elif pullback_from_low > 0.5 * atr and pullback_from_low < 2.0 * atr:
            direction = "short"
            confidence = round(min(0.9, 0.5 + pullback_from_low / atr * 0.2), 3)
        else:
            return None
        entry = cur
        return {
            ...
            "reasoning": f"Gamma scalp: pullback from high={pullback_from_high:.4f}, low={pullback_from_low:.4f}",
        }
