...
class CrossSectionalMomentumStrategy(BaseStrategy):
    """Cross-sectional momentum using recent returns vs rolling std."""

    _STRATEGY_NAME = "cross_sectional_momentum"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        roll_std = ret_20.rolling(10).std()
        ret_5 = close.pct_change(5)
        r5 = float(ret_5.iloc[-1])
        std20 = float(roll_std.iloc[-1])
        if np.isnan(r5) or np.isnan(std20) or std20 == 0:
            return None
        threshold = 1.5 * std20
        if r5 > threshold:
            direction = "long"
            ...
            return None
        confidence = round(min(0.9, 0.5 + min(1.0, abs(r5) / (threshold + 1e-9) - 1.0) * 0.3), 3)
        atr = self._calculate_atr(df)
        entry = float(close.iloc[-1])
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"XS momentum ret5={r5:.4f}, threshold={threshold:.4f}",
        }
