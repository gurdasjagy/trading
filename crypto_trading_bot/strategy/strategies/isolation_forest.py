...
class IsolationForestStrategy(BaseStrategy):
    """Anomaly detection using z-score of volume and price change."""

    _STRATEGY_NAME = "isolation_forest"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        z_vol = (volume - volume.rolling(20).mean()) / (volume.rolling(20).std() + 1e-9)
        z_price = (price_chg - price_chg.rolling(20).mean()) / (price_chg.rolling(20).std() + 1e-9)
        zv = float(z_vol.iloc[-1])
        zp = float(z_price.iloc[-1])
        if np.isnan(zv) or np.isnan(zp):
            return None
        if abs(zv) > 2.5 and abs(zp) > 2.0:
            direction = "long" if zp > 0 else "short"
            confidence = round(min(0.9, 0.5 + (abs(zv) - 2.5) * 0.1 + (abs(zp) - 2.0) * 0.1), 3)
        else:
            return None
        ...
        return {
            ...
            "reasoning": f"Anomaly detected: z_vol={zv:.2f}, z_price={zp:.2f}",
        }
