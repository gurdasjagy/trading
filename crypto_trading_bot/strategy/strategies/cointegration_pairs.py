...
class CointegrationPairsStrategy(BaseStrategy):
    """Statistical arbitrage via rolling z-score mean reversion (cointegration proxy)."""

    _STRATEGY_NAME = "cointegration_pairs"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "reasoning": f"Cointegration z-score={z:.2f}, direction={direction}",
        }

    async def generate_signal(self, symbol: str) -> Signal:
        try:
            df = await self._get_ohlcv(symbol, self._timeframe, limit=110)
    ...
    async def should_close(self, position: Any, data: Dict[str, Any]) -> bool:
        ...
        df = await self._get_ohlcv(symbol, limit=110)
        if df.empty:
            return False
        sig = self.analyze(df, symbol)
        if sig is None:
