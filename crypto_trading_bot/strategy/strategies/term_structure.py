...
class TermStructureStrategy(BaseStrategy):
    """Term structure proxy via price trend consistency."""

    _STRATEGY_NAME = "term_structure"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        positive = (ret > 0).rolling(10).sum()
        negative = (ret < 0).rolling(10).sum()
        pos = float(positive.iloc[-1])
        neg = float(negative.iloc[-1])
        if np.isnan(pos) or np.isnan(neg):
            return None
        if pos >= 7:
            direction = "long"
            confidence = round(min(0.9, 0.5 + (pos - 7) * 0.1), 3)
        elif neg >= 7:
            direction = "short"
            confidence = round(min(0.9, 0.5 + (neg - 7) * 0.1), 3)
        else:
            return None
        ...
        return {
            ...
            "reasoning": f"Term structure: pos_days={pos}, neg_days={neg}",
        }
