...
class AuctionTheoryStrategy(BaseStrategy):
    """Auction theory: initiative breakout vs responsive reversal."""

    _STRATEGY_NAME = "auction_theory"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        initiative_up = cur_close > prev_high
        initiative_down = cur_close < prev_low
        if initiative_up:
            direction = "long"
            breakout = cur_close - prev_high
            atr = self._calculate_atr(df)
            confidence = round(min(0.9, 0.5 + breakout / (atr + 1e-9) * 0.2), 3)
        elif initiative_down:
            direction = "short"
            breakout = prev_low - cur_close
            atr = self._calculate_atr(df)
            confidence = round(min(0.9, 0.5 + breakout / (atr + 1e-9) * 0.2), 3)
        else:
            return None
        ...
        return {
            ...
            "strategy": self.name, "entry_price": cur_close, "atr": atr,
            "reasoning": f"Auction initiative: cur={cur_close:.4f}, prev_high={prev_high:.4f}, prev_low={prev_low:.4f}",
        }
