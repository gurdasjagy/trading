...
class NNEnsembleStrategy(BaseStrategy):
    """Neural network ensemble proxy using EMA crossovers at multiple lookbacks."""

    _STRATEGY_NAME = "nn_ensemble"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        for w in self._windows:
            if len(close_list) < w * 2:
                continue
            ema_fast = self._calculate_ema(close_list, w)
            ema_slow = self._calculate_ema(close_list, w * 2)
            votes.append(1 if ema_fast > ema_slow else -1)
        if not votes:
            return None
        score = float(sum(votes)) / len(votes)
        if score > 0.3:
            direction = "long"
        elif score < -0.3:
            direction = "short"
            ...
            return None
        confidence = round(min(0.9, 0.5 + abs(score) * 0.4), 3)
        atr = self._calculate_atr(df)
        entry = float(close.iloc[-1])
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"NN ensemble score={score:.2f} from {len(votes)} voters",
        }
