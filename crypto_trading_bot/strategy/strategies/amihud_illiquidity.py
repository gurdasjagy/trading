...
class AmihudIlliquidityStrategy(BaseStrategy):
    """Amihud illiquidity ratio — trade only in liquid conditions."""

    _STRATEGY_NAME = "amihud_illiquidity"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        amihud = log_ret / (dollar_vol + 1e-9)
        pct = amihud.rolling(20).mean().rank(pct=True)
        p = float(pct.iloc[-1])
        if np.isnan(p) or p > 0.8:
            return None
        ret5 = float(close.pct_change(5).iloc[-1])
        if np.isnan(ret5):
            return None
        direction = "long" if ret5 > 0 else "short"
        confidence = round(min(0.9, 0.5 + (0.8 - p) * 0.5), 3)
        atr = self._calculate_atr(df)
        entry = float(close.iloc[-1])
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"Amihud pct={p:.2f}, liquid market momentum",
        }
