...
class DollarBarsStrategy(BaseStrategy):
    """Dollar bar sampling proxy strategy."""

    _STRATEGY_NAME = "dollar_bars"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        ...
        threshold = float(dollar_vol.rolling(20).mean().iloc[-1])
        if np.isnan(threshold) or threshold == 0:
            return None
        roll_dv = dollar_vol.rolling(5).sum()
        bar_complete = roll_dv > threshold
        if not bool(bar_complete.iloc[-1]):
            return None
        ret = float(close.pct_change(5).iloc[-1])
        if np.isnan(ret):
            return None
        direction = "long" if ret > 0 else "short"
        dv_ratio = float(roll_dv.iloc[-1]) / threshold
        confidence = round(min(0.9, 0.5 + (dv_ratio - 1.0) * 0.15), 3)
        atr = self._calculate_atr(df)
        entry = float(close.iloc[-1])
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"Dollar bar complete, momentum ret5={ret:.4f}",
        }
