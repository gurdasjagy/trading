...
class StraddleReplicationStrategy(BaseStrategy):
    """Breakout after low-volatility consolidation (straddle replication)."""

    _STRATEGY_NAME = "straddle_replication"
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "") -> Optional[Dict[str, Any]]:
        if len(df) < self._min_rows:
            return None
        close = df["close"]
        mean10 = float(close.rolling(10).mean().iloc[-1])
        cur = float(close.iloc[-1])
        if np.isnan(mean10) or mean10 == 0:
            return None
        flat = abs(cur - mean10) / mean10 < 0.005
        if flat:
            return None
        direction = "long" if cur > mean10 else "short"
        deviation = abs(cur - mean10) / mean10
        confidence = round(min(0.9, 0.5 + deviation * 10), 3)
        atr = self._calculate_atr(df)
        entry = cur
        return {
            "symbol": symbol, "direction": direction, "confidence": confidence,
            "strategy": self.name, "entry_price": entry, "atr": atr,
            "reasoning": f"Straddle breakout: cur={cur:.4f}, mean10={mean10:.4f}, dev={deviation:.4f}",
        }
